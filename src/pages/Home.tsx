...
export default function Home() {
  ...
  const checkUser = async () => { 
    ...
    if (user?.is_anonymous) { setIsAnonymous(true) } else { setCurrentUserEmail(user?.email) }
  }
  ...
  // 通用计算逻辑：生成遮罩数据
  const createMask = (clientX: number, clientY: number) => {
    if (!imgRef.current) return
    const rect = imgRef.current.getBoundingClientRect()
    const w = Math.abs((clientX - rect.left) - startPos.x)
    const h = Math.abs((clientY - rect.top) - startPos.y)
    
    // 只有当拖拽距离大于 5px 时才认为是在画框（防止误触）
    if (w > 5 && h > 5) {
      setMasks([...masks, { 
        id: Date.now().toString(), 
        x: (Math.min(clientX - rect.left, startPos.x) / rect.width) * 100, 
        y: (Math.min(clientY - rect.top, startPos.y) / rect.height) * 100, 
        w: (w / rect.width) * 100, 
        h: (h / rect.height) * 100 
      }])
    }
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    if (!isDrawing) return
    // 获取离开屏幕时的手指位置 (changedTouches)
    const touch = e.changedTouches[0]
    createMask(touch.clientX, touch.clientY)
    setIsDrawing(false)
  }
  ...
  const handleUpgradeAccount = async (e: React.FormEvent) => { e.preventDefault(); setIsRegistering(true); const { error } = await supabase.auth.updateUser({ email: registerEmail, password: registerPassword }); if (error) { alert(`升级失败: ${error.message}`) } else { setRegisterSuccess(true) }; setIsRegistering(false) }
  const handleUpdatePassword = async (e: React.FormEvent) => { e.preventDefault(); if (!registerPassword) return; setIsRegistering(true); const { error } = await supabase.auth.updateUser({ password: registerPassword }); if (error) { alert(`设置失败: ${error.message}`) } else { alert("🎉 密码设置成功！"); setShowSettingsModal(false); setRegisterPassword('') }; setIsRegistering(false) }
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl p-6 relative">
            ...
            <div className="text-center"><div className="w-12 h-12 bg-gray-100 text-gray-600 rounded-full flex items-center justify-center mx-auto mb-4"><User className="w-6 h-6" /></div><h3 className="text-xl font-black text-gray-900 mb-1">账号设置</h3><p className="text-gray-500 text-sm mb-6">当前账号: {currentUserEmail}</p><form onSubmit={handleUpdatePassword} className="space-y-4 text-left"><div className="relative"><Lock className="w-5 h-5 text-gray-400 absolute left-4 top-3.5" /><input type="password" required placeholder="设置新密码" minLength={6} value={registerPassword} onChange={e => setRegisterPassword(e.target.value)} className="w-full bg-gray-50 border border-gray-200 pl-11 pr-4 py-3 rounded-xl outline-none focus:border-black font-medium text-gray-900" /></div><button type="submit" disabled={isRegistering} className="w-full bg-black text-white py-3 rounded-xl font-bold hover:bg-gray-800 transition-all flex items-center justify-center gap-2">{isRegistering ? <Loader2 className="w-5 h-5 animate-spin" /> : '更新密码'}</button></form></div>
          </div>
        </div>
      )}
      ...
      <div className="p-6 space-y-8">
        <header className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            ...
            {!isAnonymous && (<button onClick={() => setShowSettingsModal(true)} className="p-2 text-gray-400 hover:text-black hover:bg-gray-100 rounded-full transition-colors"><Settings className="w-5 h-5" /></button>)}
             ...
             <div className="w-full bg-white p-6 rounded-3xl shadow-lg border border-indigo-50">
                ...
                {imagePreview ? (
                  <div className="relative mb-4 border-2 border-dashed border-indigo-100 rounded-lg overflow-hidden select-none touch-none">
                    {/* ✨ 画图感应区：增加了 onTouchStart, onTouchEnd 和 touch-none */}
                    <div 
